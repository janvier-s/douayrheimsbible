<script lang="ts">
	...
	function linkify(html: string): string {
		let result = linkifyItalicRefs(html, true);
		// Unwrap short italic number patterns (Confraternity chapter notation: <i>13,</i>)
		// so the bare-ref tokenizer can see complete references
		result = result.replace(/<i>(\d+[,.]?\s*)<\/i>/g, '$1');
		return linkifyBareRefs(result);
	}
...
<svelte:head>
	...
	<link
		rel="canonical"
		href="https://thedouayrheims.com/reference/{translation}/{section}/{article.slug}"
	/>
</svelte:head>
...
>
	<header class="ref-page-header" class:ref-page-header--centered={isTitlePage}>
		<a href="/reference/{translation}" class="ref-eyebrow">
	...
	{#if prev || next}
		<nav class="ref-article-nav ref-article-nav--top" aria-label="Article navigation (top)">
			{#if prev}
				<a
					href="/reference/{translation}/{prev.section}/{prev.slug}"
			...
			{#if next}
				<a
					href="/reference/{translation}/{next.section}/{next.slug}"
	...
	{#if hasToc}
		<nav class="ref-toc" aria-label="Table of contents">
			<p class="ref-toc-heading">Contents</p>
			<ol class="ref-toc-list">
				{#each content.toc as item}
					<li><a href="#{item.id}" class="ref-toc-link">{item.label}</a></li>
				{/each}
			</ol>
		</nav>
	{/if}
	...
	>
		{#if contentType === 'paragraphs' && isPlainParagraphs}
			...
			{#each content.paragraphs as para}
				{#if /^[-_]{5,}$/.test(para.trim())}
					<hr class="ref-hr" />
				{:else}
					<div class="ref-prose-block">
						<p class="ref-para">
							{@html skipLinkify ? allcapsToSmallcaps(para) : linkify(allcapsToSmallcaps(para))}
						</p>
					</div>
				{/if}
			{/each}
		{:else if contentType === 'paragraphs'}
			<!-- Structured paragraphs with optional notes (ODR-style) -->
		...
		{#if (isTitlePage || isLastPage) && (section === 'ot' || section === 'nt')}
			<div class="ref-cta">
				<a href="/odr/{section === 'ot' ? 'genesis' : 'matthew'}/1" class="ref-cta-btn">
					Read {section === 'ot' ? 'Old' : 'New'} Testament
				</a>
			</div>
		{:else if isLastPage && translation === 'conf'}
			<div class="ref-cta">
				<a href="/conf/matthew/1" class="ref-cta-btn"> Read the Confraternity New Testament </a>
			</div>
		{:else if isLastPage && translation === 'drc'}
			<div class="ref-cta">
				<a href="/drc/genesis/1" class="ref-cta-btn"> Read the Douay-Rheims Challoner </a>
			</div>
		{:else if isLastPage && translation === 'haydock'}
			<div class="ref-cta">
				<a href="/haydock/genesis/1" class="ref-cta-btn"> Read with Haydock Commentary </a>
			</div>
		{/if}
...
<style>
	...
	/* Anchored section headings inside {@html} */
	.ref-body :global(h2[id]) {
		font-family: var(--font-reader);
		font-size: 1.3rem;
		font-weight: 700;
		color: var(--color-heading, var(--color-text));
	...
	/* Paragraph-initial bold with anchor (Conf-style sections) */
	.ref-body :global(b[id]) {
		scroll-margin-top: 120px;
	}
	...
	/* ── Psalm numbering grid ────────────────────────────────── */
	.ref-body :global(.psalm-grid) {
		display: grid;
	...
	.ref-body :global(.psalm-grid span) {
		padding: 4px 8px;
