...
import type {
	...
	FathersPericope,
	FathersChapterFile
} from '../src/lib/data/fathers-types';
...
for (const [slugChapter, pericopeMap] of bySlugChapter) {
	...
	const sorted = [...pericopeMap.entries()].sort((a, b) => a[1].startVerse - b[1].startVerse);

	for (const [
		verseRef,
		{ startVerse, endVerse, pericopeTitle, overview, entries: perEntries }
	] of sorted) {
		pericopes.push({
			verseRef,
			startVerse,
